...
final class CaptureEngine: NSObject, @unchecked Sendable {
    ...
    private func composite(screenBuffer: CVPixelBuffer) -> CVPixelBuffer? {
        guard let adaptor = videoAdaptor, let pool = adaptor.pixelBufferPool else { return nil }
        var outputBuffer: CVPixelBuffer?
        guard CVPixelBufferPoolCreatePixelBuffer(nil, pool, &outputBuffer) == kCVReturnSuccess, let outputBuffer else { return nil }
...
extension CaptureEngine: SCStreamOutput, SCStreamDelegate {
    nonisolated func stream(_ stream: SCStream, didOutputSampleBuffer sampleBuffer: CMSampleBuffer, of type: SCStreamOutputType) {
        ...
        guard let screenBuffer = sampleBuffer.imageBuffer,
              let input = videoInput,
              let adaptor = videoAdaptor,
              input.isReadyForMoreMediaData,
              let outputBuffer = composite(screenBuffer: screenBuffer) else { return }
